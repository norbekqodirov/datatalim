...
const EMPTY: Partial<StaffMember> = {
  name: '', phone: '', role: 'Operator', salary_type: 'fixed', salary_amount: 0, status: 'active'
};
...
export default function Staff() {
  ...
  const [form, setForm] = useState<Partial<StaffMember>>(EMPTY);
  ...
  const filtered = useMemo(() => {
    ...
    return staff.filter(s => s.name.toLowerCase().includes(q) || s.phone.includes(q) || s.role.toLowerCase().includes(q));
  }, [staff, search]);

  const openAdd = () => { setForm(EMPTY); setEditId(null); setShowForm(true); };
  const openEdit = (s: StaffMember) => { setForm({ ...s }); setEditId(s.id); setShowForm(true); };
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          <h1 className={`text-2xl font-black flex items-center gap-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>
            <Users className="text-emerald-500" size={26} /> Xodimlar
          </h1>
          <p className={`text-sm mt-0.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Markaz xodimlarini boshqarish</p>
        </div>
        <div className="flex gap-2">
          <button onClick={fetchStaff} className={`p-2.5 rounded-xl border transition-colors ${isDark ? 'border-white/10 text-slate-400 hover:bg-slate-800' : 'border-slate-200 text-slate-500 hover:bg-slate-50'}`}>
            <RefreshCw size={16} className={loading ? 'animate-spin' : ''} />
          </button>
          <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-sm transition-colors shadow-lg shadow-emerald-500/25">
      ...
      <div className={`flex gap-3 p-3 rounded-2xl ${card}`}>
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Ism, telefon yoki lavozim..." className={inp + ' pl-9'} />
        </div>
      </div>
      ...
      <div className={`rounded-2xl border overflow-hidden ${isDark ? 'border-white/10' : 'border-slate-200'}`}>
        {loading ? (
          ...
          <div className={`text-center py-16 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
            ...
            <p className="font-medium">Xodim topilmadi</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className={`${isDark ? 'bg-slate-900/50 text-slate-400' : 'bg-slate-50 text-slate-500'} text-xs font-bold uppercase tracking-wider`}>
                <tr>
                  ...
                  <th className="px-4 py-3 text-left">Oylik (Stavka)</th>
              ...
              <tbody className="divide-y divide-white/5">
                {filtered.map((s) => (
                  <tr key={s.id} className={`transition-colors ${isDark ? 'hover:bg-white/5' : 'hover:bg-slate-50'}`}>
                    ...
                    <td className="px-4 py-3 text-slate-700 dark:text-slate-200">{s.role}</td>
                    <td className="px-4 py-3 font-medium text-emerald-600 dark:text-emerald-400">
                      {s.salary_type === 'fixed' ? `${s.salary_amount.toLocaleString()} so'm` : `${s.salary_amount}% (Foiz)`}
                    </td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded-lg text-xs font-bold ${s.status === 'active' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}>
                        {s.status === 'active' ? 'Faol' : 'Nofaol'}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => openEdit(s)} className="p-1.5 rounded-lg transition-colors text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-slate-800"><Edit3 size={15} /></button>
                        <button onClick={() => handleDelete(s.id)} className="p-1.5 rounded-lg text-red-400 hover:bg-red-500/10 transition-colors"><Trash2 size={15} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.93, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.93, opacity: 0 }} onClick={e => e.stopPropagation()} className={`w-full max-w-lg rounded-3xl shadow-2xl ${isDark ? 'bg-slate-900 border border-slate-700' : 'bg-white border border-slate-200'}`}>
              <div className={`flex items-center justify-between p-6 border-b ${isDark ? 'border-slate-700' : 'border-slate-200'}`}>
                <h2 className={`text-lg font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>{editId ? "Xodimni tahrirlash" : "Yangi xodim qo'shish"}</h2>
              ...
              <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div><label className={lbl}>Lavozim</label><input value={form.role||''} onChange={e=>setForm(f=>({...f,role:e.target.value}))} className={inp} placeholder="Operator" /></div>
                
                <div>
                  <label className={lbl}>Oylik turi</label>
                  <select value={form.salary_type||'fixed'} onChange={e=>setForm(f=>({...f,salary_type:e.target.value as any}))} className={inp}>
                    <option value="fixed">Fiks (Bir xil oylik)</option>
                    <option value="percentage">Foiz (%)</option>
                  </select>
                </div>
                <div><label className={lbl}>Summa yoki Foiz</label><input type="number" value={form.salary_amount||0} onChange={e=>setForm(f=>({...f,salary_amount:Number(e.target.value)}))} className={inp} /></div>
