...
export const Contact: React.FC = () => {
  ...
  return (
    <div id="contact" className="py-24 relative overflow-hidden" style={{ background: isDark ? '#000' : '#fff' }}>
      ...
      <div className="absolute top-1/4 -right-32 w-96 h-96 bg-[#0061ff] blur-[100px] opacity-10 rounded-full pointer-events-none"></div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 relative">
          <Star2 size={60} color="#0061ff" opacity={0.1} className="absolute -top-10 left-10 rotate-slow pointer-events-none hidden lg:block" />

          <div>
            <h2 className={`text-4xl md:text-5xl font-black mb-6 uppercase tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>
              Biz bilan <span className="text-transparent bg-clip-text shimmer-text" style={{ backgroundImage: 'linear-gradient(135deg, #0061ff, #60efff)' }}>bog'laning</span>
            </h2>
            ...
            <div className="space-y-6">
              ...
              {[
                ...
                <div key={i} className={`flex items-start gap-5 p-6 rounded-3xl border transition-all hover:translate-x-2 group ${isDark ? 'bg-slate-900/50 border-white/5 hover:bg-slate-800/80 hover:shadow-xl hover:shadow-[#0061ff]/20' : 'bg-slate-50 border-slate-100 hover:bg-white hover:shadow-xl hover:shadow-[#0061ff]/10'}`}>
                  <div className={`p-4 rounded-2xl shrink-0 transition-transform group-hover:scale-110 group-hover:rotate-3 ${isDark ? 'bg-[#0061ff]/20 text-[#60efff]' : 'bg-blue-100 text-[#0061ff]'}`}>
                    <item.icon size={26} />
                  </div>
          ...
          <div className="relative">
            <Star1 size={80} color="#60efff" opacity={0.05} className="absolute -top-10 right-0 rotate-slow pointer-events-none" />
            <div className={`p-8 md:p-10 rounded-[2rem] border shadow-2xl relative overflow-hidden backdrop-blur-xl ${isDark ? 'bg-slate-900/80 border-white/10 shadow-black/50' : 'bg-white/80 border-slate-100 shadow-blue-900/5'}`}>
              {/* Subtle gradient border effect */}
              <div className="absolute top-0 right-0 w-32 h-32 opacity-20" style={{ background: 'linear-gradient(135deg, #60efff, transparent)' }}></div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-5 py-4 rounded-2xl border-2 outline-none transition-all font-medium ${isDark ? 'bg-slate-800 border-slate-700 focus:border-[#0061ff] text-white placeholder-slate-500 focus:bg-slate-900' : 'bg-slate-50 border-slate-200 focus:border-[#0061ff] focus:ring-4 focus:ring-[#0061ff]/10 text-slate-900 focus:bg-white'}`}
                  ...
                  <div>
                    ...
                    <div className={`flex w-full overflow-hidden rounded-2xl border-2 transition-all font-medium focus-within:ring-4 focus-within:ring-[#0061ff]/10 ${isDark ? 'bg-slate-800 border-slate-700 focus-within:border-[#0061ff] text-white focus-within:bg-slate-900' : 'bg-slate-50 border-slate-200 focus-within:border-[#0061ff] text-slate-900 focus-within:bg-white'}`}>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={handleChange}
                      className={`w-full px-5 py-4 rounded-2xl border-2 outline-none transition-all h-36 resize-none font-medium text-base ${isDark ? 'bg-slate-800 border-slate-700 focus:border-[#0061ff] text-white placeholder-slate-500 focus:bg-slate-900' : 'bg-slate-50 border-slate-200 focus:border-[#0061ff] focus:ring-4 focus:ring-[#0061ff]/10 text-slate-900 focus:bg-white'}`}
